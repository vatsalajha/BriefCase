...
function midpageHeaders(): Record<string, string> {
  const key = process.env.MIDPAGE_API_KEY;
  if (!key) throw new Error("MIDPAGE_API_KEY environment variable is not set.");
  return {
...
interface PostGetResponse {
  opinions: MidpageOpinion[];
  citation_matches: { citation: string; opinion_id: string }[];
}
 ...
 */
async function lookupByCitation(citation: string): Promise<MidpageOpinion | null> {
  ...
  const json = await res.json() as PostGetResponse;
 ...
 */
async function getCitatorTreatment(opinionId: string): Promise<string | null> {
...
function treatmentBadge(treatment: string | null): string {
  if (!treatment) return "Not available";
  if (treatment === "Negative") return "⚠️ Negative (may be overruled or distinguished)";
  if (treatment === "Caution") return "⚠️ Caution (treated with caution by later courts)";
  return `✅ ${treatment}`;
}
...
server.tool(
  "verify_citation",
  "Verify a legal case citation against the Midpage case law database. Returns whether the case exists, its court, date, and citator treatment (positive/negative/caution). Use this whenever a user mentions a case citation.",
  {
    citation: z.string().describe(
      "The reporter citation to verify, e.g. '410 U.S. 113' or '384 U.S. 436'"
    ),
  },
  async ({ citation }) => {
    try {
...
const CITATION_RE =
  /\b(\d{1,4})\s+(U\.S\.|F\.\d[a-z]*|F\.\s*Supp\.(?:\s*\d[a-z]*)?|S\.(?:\s*)Ct\.|L\.\s*Ed\.(?:\s*\d[a-z]*)?|A\.\d[a-z]*|Cal\.\d[a-z]*|N\.Y\.\d[a-z]*)\s+(\d{1,4})\b/g;

interface ParsedCitation {
  raw: string;
  normalized: string; // "volume reporter page"
}

function extractCitations(text: string): ParsedCitation[] {
...
server.tool(
  "check_brief",
  "Extract and verify all case law citations from a legal brief or document. Returns a full verification report for every citation found — whether it exists, its treatment, and any warnings.",
  {
